...
namespace NNPEFWEB.Service
{
        ...
        {
            using IDbConnection db = new SqlConnection(_config.GetConnectionString(Connectionstring));
            IEnumerable<T> queryResp= await db.QueryAsync<T>(sp, parms, commandType: commandType);
            return queryResp.ToList();
        }
        ...
        public async Task<T> GetAsync<T>(string sp, DynamicParameters parms, CommandType commandType = CommandType.StoredProcedure)
        {
            ...
            IEnumerable<T> queryResponse=await db.QueryAsync<T>(sp, parms, commandType: commandType);
